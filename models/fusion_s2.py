"""Stage-2 RGB F2R fusion network with deformable alignment."""
...
class FlowGuidedBlock(nn.Module):
    """
    Stage 2: Flow-guided feature alignment block using Deformable Convolutions.
    Learns dynamic offsets using optical flow as a prior.
    """
    ...
    def forward(self, inp, flow):
        ref_index = (self.T + 1) // 2
        not_ref = [i for i in range(self.T + 1) if i != ref_index]

        x = rearrange(inp, '(b t) c h w -> b t c h w', t=self.T + 1)
        ref = x[:, ref_index]
        x = rearrange(x[:, not_ref], 'b t c h w -> (b t) c h w')

        x_warp = flow_warp(x, flow, mode='bilinear')

        # Calculate deformable offsets
...
class Fusion(nn.Module):
    """
    Stage 2 Fusion Network based on U-Net architecture.
    Refines temporal alignment using Deformable Convolutions.
    """
