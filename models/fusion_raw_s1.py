"""Stage-1 RAW F2R fusion network for packed GBRG RAW videos."""
...
class FlowGuidedBlock(nn.Module):
    def __init__(self, c, t):
        super().__init__()
        self.T = t
        self.conv1 = nn.Conv2d(in_channels=c*t, out_channels=c*t, kernel_size=1, padding=0, stride=1, groups=1, bias=True)
        self.conv2 = nn.Conv2d(in_channels=c*t, out_channels=c*t, kernel_size=3, padding=1, stride=1, groups=c*t, bias=True)
        self.conv3 = nn.Conv2d(in_channels=c*t, out_channels=c, kernel_size=1, padding=0, stride=1, groups=1, bias=True)
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_channels=c*t, out_channels=c*t // 2, kernel_size=1, padding=0, stride=1, groups=1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=c*t // 2, out_channels=c*t, kernel_size=1, padding=0, stride=1, groups=1, bias=True),
            nn.Sigmoid()
        )
...
class Fusion(nn.Module):
    ...
    def forward(self, inp, coarse_img, flow):
        ...
        # Level 2
        inp_enc_level2 = self.down1_2(out_enc_level1)  # [Bt, c2, H/2, W/2]
        out_enc_level2 = self.lrelu(self.encoder_level2(inp_enc_level2))
        # Level 3
        inp_enc_level3 = self.down2_3(out_enc_level2)  # [Bt, c3, H/4, W/4]
        out_enc_level3 = self.lrelu(self.encoder_level3(inp_enc_level3))
        # Level 4
        inp_enc_level4 = self.down3_4(out_enc_level3)  # [Bt, c4, H/8, W/8]
        # ---------- Decoder ----------
