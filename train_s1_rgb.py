"""Train Stage-1 RGB F2R under synthetic Gaussian/blind noise."""
...
# === F2R Module Imports ===
from models.fusion_s1 import Fusion
...
parser.add_argument('--n_snapshot', type=int, default=1, help='Validate and save every N epochs')

parser.add_argument('--log_name', type=str, default='f2r_rgb_s1')
...
def flow_to_ref(coarse, estimator, ref_idx):
    ...
    for i in range(T):
        if i == ref_idx:
            continue
        if i < ref_idx:
            _, flow_ir = get_flow_2frames_train(estimator,
                                                torch.cat([coarse[:, i].unsqueeze(1), ref_frame.unsqueeze(1)], dim=1))
        if i > ref_idx:
            flow_ir, _ = get_flow_2frames_train(estimator,
                                                torch.cat([ref_frame.unsqueeze(1), coarse[:, i].unsqueeze(1)], dim=1))
...
FusionNet = Fusion(t=int(args.len - 1), dim=args.n_features).to(device)
...
if is_ddp:
    ...
    Estimator = DDP(Estimator, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=False,
                    broadcast_buffers=True)
    FusionNet = DDP(FusionNet, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=False,
                    broadcast_buffers=True)
...
best_metrics_davis = {'epoch': 0, 'psnr': 0.0, 'ssim': 0.0, 'coarse_psnr': 0.0, 'coarse_ssim': 0.0}
best_metrics_set8 = {'epoch': 0, 'psnr': 0.0, 'ssim': 0.0, 'coarse_psnr': 0.0, 'coarse_ssim': 0.0}
...
for epoch in range(1, args.epochs + 1):
    ...
    for clean in RGBLoader:
        ...
        input = torch.cat([coarse, noisy - coarse], dim=2)

        # Forward pass
        output = FusionNet(input[:, not_ref].reshape(B * (T - 1), C * 2, H, W), coarse[:, ref_frame], f)
        loss = torch.mean(abs(output - noisy[:, ref_frame]))
    ...
    if epoch % args.n_snapshot == 0:
        ...
        if main_proc:
            ...
            torch.save(FusionNet.state_dict(), os.path.join(save_path, f'epoch_Fusion_{epoch:03d}.pth'))

            validation_path = os.path.join(args.save_path, "validation")
            os.makedirs(validation_path, exist_ok=True)

        @torch.inference_mode()
        def run_validation(loader, dataset_name, best_metrics):
            ...
            for clean in loader:
                ...
                output_padded = FusionNet(
                    input_fusion_padded[:, not_ref].reshape(B * (T - 1), C * 2, H_padded, W_padded),
            ...
            if main_proc:
                ...
                print(f"[{dataset_name}]: PSNR_1 {psnr_1:.4f}, SSIM_1 {ssim_1:.4f}, coarsePSNR {co_psnr:.4f}, coarseSSIM {co_ssim:.4f}")

                color_v_min, color_v_max = (38.6, 39.5) if dataset_name == 'Davis' else (37.3, 38.0)
                print(f"[{dataset_name} Best] @ Epoch {best_metrics['epoch']:03d}: PSNR1 {gradient_num_color(best_metrics['psnr'], v_min=color_v_min, v_max=color_v_max)}, SSIM1 {best_metrics['ssim']:.4f}")


        if epoch == args.epochs:
            run_validation(davisLoader, "Davis", best_metrics_davis)
            run_validation(set8Loader, "Set8", best_metrics_set8)
